...
public class PlayerMovement : MonoBehaviour
{
    ...
    else if (movement.z != 0)
    {
        facingDirection = new Vector3(0, 0, movement.z).normalized;
            ...
            if (Input.GetKeyDown(KeyCode.F) && !aerialAttackHandler.isOnCooldown)
            {
            ...
            if (Input.GetKey(KeyCode.Space) && !chargeAttackHandler.isOnCooldown && !isCharging)
            {
            ...
            if (Input.GetMouseButtonDown(1) && !isTailAttacking && !isTailAttackOnCooldown)
            {
